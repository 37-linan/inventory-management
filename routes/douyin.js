...
module.exports = function(db) {
  ...
  router.get('/products', (req, res) => {
    try {
      const products = db.prepare('SELECT * FROM douyin_products ORDER BY type, code').all();
      res.json(products);
    } catch (e) {
  ...
  router.get('/products/:code', (req, res) => {
    try {
      const product = db.prepare('SELECT * FROM douyin_products WHERE code = ?').get(req.params.code);
  ...
  router.post('/products', (req, res) => {
    try {
      ...
      const existing = db.prepare('SELECT id FROM douyin_products WHERE code = ?').get(code);
      if (existing) return res.status(400).json({ error: '该产品编号已存在' });

      const stmt = db.prepare('INSERT INTO douyin_products (code, name, spec, unit, market_price, type) VALUES (?, ?, ?, ?, ?, ?)');
      stmt.run(code, name, spec || '', unit || '', market_price || '', type || '');
  ...
  router.put('/products/:code', (req, res) => {
    try {
      ...
      const existing = db.prepare('SELECT id FROM douyin_products WHERE code = ?').get(req.params.code);
      if (!existing) return res.status(404).json({ error: '产品不存在' });

      db.prepare(`UPDATE douyin_products SET name=?, spec=?, unit=?, market_price=?, type=?, updated_at=datetime('now','localtime') WHERE code=?`)
  ...
  router.delete('/products/:code', (req, res) => {
    try {
      db.prepare('DELETE FROM douyin_products WHERE code = ?').run(req.params.code);
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  router.get('/price-history/:productCode', (req, res) => {
    try {
      const records = db.prepare(`
        SELECT * FROM douyin_price_history 
        WHERE product_code = ? 
        ORDER BY date DESC 
        LIMIT 7
  ...
  router.post('/price-history', (req, res) => {
    try {
      ...
      const existing = db.prepare('SELECT id FROM douyin_price_history WHERE product_code = ? AND date = ?').get(product_code, today);
      if (existing) {
        db.prepare('UPDATE douyin_price_history SET price = ? WHERE product_code = ? AND date = ?').run(price, product_code, today);
      } else {
        db.prepare('INSERT INTO douyin_price_history (product_code, date, price) VALUES (?, ?, ?)').run(product_code, today, price);
      }
  ...
  router.get('/inbound', (req, res) => {
    try {
      const records = db.prepare('SELECT * FROM douyin_inbound ORDER BY created_at DESC').all();
      res.json(records);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  router.post('/inbound', (req, res) => {
    try {
      ...
      const stmt = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `);
      stmt.run(product_code, quantity, device || '', channel || '', remark || '', order_no || '', image_path || '', purchase_price || 0);
  ...
  router.delete('/inbound/:id', (req, res) => {
    try {
      db.prepare('DELETE FROM douyin_inbound WHERE id = ?').run(req.params.id);
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  router.get('/outbound', (req, res) => {
    try {
      const records = db.prepare('SELECT * FROM douyin_outbound ORDER BY created_at DESC').all();
      res.json(records);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  router.post('/outbound', (req, res) => {
    try {
      ...
      const stmt = db.prepare(`
        INSERT INTO douyin_outbound (product_code, quantity, image_path, location) 
        VALUES (?, ?, ?, ?)
      `);
  ...
  router.delete('/outbound/:id', (req, res) => {
    try {
      db.prepare('DELETE FROM douyin_outbound WHERE id = ?').run(req.params.id);
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  router.get('/inventory', (req, res) => {
    try {
      ...
      const outboundSummary = db.prepare(`
        ...
        FROM douyin_outbound 
        GROUP BY product_code
      `).all();

      const products = db.prepare('SELECT * FROM douyin_products').all();
  ...
  router.get('/config/:key', (req, res) => {
    try {
      const cfg = db.prepare('SELECT config_value FROM douyin_config WHERE config_key = ?').get(req.params.key);
  ...
  router.put('/config/:key', (req, res) => {
    try {
      ...
      const existing = db.prepare('SELECT id FROM douyin_config WHERE config_key = ?').get(req.params.key);
      if (existing) {
        db.prepare('UPDATE douyin_config SET config_value = ? WHERE config_key = ?').run(JSON.stringify(value), req.params.key);
      } else {
        db.prepare('INSERT INTO douyin_config (config_key, config_value) VALUES (?, ?)').run(req.params.key, JSON.stringify(value));
      }
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
